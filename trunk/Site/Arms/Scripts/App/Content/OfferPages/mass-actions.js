define('Content/OfferPages/mass-actions', [], function () {
    return function (options) {

        // --------------- Данные

        var list = $(options.list),
            actionsPanel = $('[data-x="content/offerpages/actions_panel"]'),
            counterNode = $('[data-x="content/offerpages/counter_node"]'),
            selectAllBtn = $('[data-x="content/offerpages/select_all"]'),
            selectedPages = [];
        ...
        list.on('change', '[name="offer"]', function () {
            registerSelection(this.checked, [this.value]);
        });

        selectAllBtn.on('change', function () {
            if (this.checked) {
                var ids = list.find('[name="offer"]').map(function () {
                    return $(this).val();
                }).toArray();
                list.find('[name="offer"]').prop('checked', true);
        ...
        function updateActionsPanel() {
            ...
            if (count) {
                ...
                var text = Globalize.pluralize(
                    count,
                    'Отмечена {1:n0} оферта',
                    'Отмечено {2:n0} оферты',
                    'Отмечено {5:n0} оферт'
                );
        ...
        function resetSelection() {
            selectedPages = [];
            list.find('[name="offer"]').prop('checked', false);
