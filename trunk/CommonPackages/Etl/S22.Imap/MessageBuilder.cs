...
namespace S22.Imap {
	...
	public static class MessageBuilder {
			...
			try {
				switch (part.Encoding) {
					...
					case ContentTransferEncoding.Base64:
						...
						break;
                    case ContentTransferEncoding.UUEncoding:
                        bytes = Encoding.GetEncoding(1251).GetBytes(content);
                        break;
					default:
		...
		private static Bodypart BodypartFromMIME(MIMEPart mimePart) {
            ...
            // Обработка вложений в UUEncoding
            if (mimePart.body != null && mimePart.body.StartsWith("begin"))
            {      
                var startIndex = "begin 644 ".Length;
                var length = mimePart.body.IndexOf('\r') - startIndex;
                var filename = mimePart.body.Substring(startIndex, length);

                p.Type = ContentType.Application;
                p.Subtype = "octet-stream";
                p.Encoding = ContentTransferEncoding.UUEncoding;
                p.Parameters = new Dictionary<string, string>();
                p.Parameters.Add("name", filename);
                p.Parameters.Add("value", "octet-stream");
                p.Disposition = new ContentDisposition();
                p.Disposition.Type = ContentDispositionType.Attachment;
                p.Disposition.Filename = filename;
                p.Disposition.Attributes = new Dictionary<string, string>();
                p.Disposition.Attributes.Add("fileame", filename);
                p.Disposition.Attributes.Add("value", "attachment");

                mimePart.body = mimePart.body.Substring(9).Replace(filename, "").Replace("end", "").Trim('\r', '\n', ' ');
                mimePart.body = DecodeFromUU(mimePart.body);
        ...
        private static string DecodeFromUU(string str)
        {
            str = "\r\n" + str;
            bool done = false;
            int cur;

            // Удаляем символы конца строки и первый символ каждой строки 
            // (в нем хранится длина строки - не более 80 символов)
            while (!done)
            {
                cur = str.IndexOf('\r');
                if (cur < 0)
                    ...
                    str = str.Remove(cur, 3);
            }

            var bytes = new List<byte>();
            byte[] asciiBytes = Encoding.ASCII.GetBytes(str);
            var asciiList = new List<byte>(asciiBytes);

            // выравниваем список байт до числа, которое делится на 4
            for (int j = 0; j < 8 - asciiBytes.Length % 4; j++)
                asciiList.Add(32);

            for (int i = 0; i < asciiList.Count; i++)
            {
                asciiList[i] = (byte)((asciiList[i] - 32) & 63);
            }

            for (int i = 0; i < asciiList.Count; i += 4)
            {
                bytes.Add((byte)(asciiList[i] << 2 | asciiList[i + 1] >> 4));
                bytes.Add((byte)(asciiList[i + 1] << 4 | asciiList[i + 2] >> 2));
                bytes.Add((byte)(asciiList[i + 2] << 6 | asciiList[i + 3]));
            }
