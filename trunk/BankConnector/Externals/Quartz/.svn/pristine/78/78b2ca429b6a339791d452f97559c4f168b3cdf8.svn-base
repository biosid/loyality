...
namespace Quartz.Simpl
{
    ...
    public class RAMJobStore : IJobStore
    {
        ...
        private List<TriggerWrapper> GetTriggerWrappersForJobGroup(string @group)
        {
            var trigList = new List<TriggerWrapper>();
            ...
            lock (lockObject)
            {
                ...
                {
                    TriggerWrapper tw = triggers[i];
                    if (tw.jobKey.Group.Equals(@group))
                    {
                        trigList.Add(tw);
                    }
                }
        ...
        public virtual IList<IOperableTrigger> AcquireNextTriggers(DateTimeOffset noLaterThan, int maxCount, TimeSpan timeWindow)
        {
            lock (lockObject)
            {
                ...
                while (true)
                {
                    ...
                    // NOTE: Проверяем запрет на конкуренцию по группе.
                    ...
                    {
                        // NOTE: Проверяем запрет на конкуренцию по jobKey (название и группе), только если нет запрета по группе.
        ...
        public virtual IList<TriggerFiredResult> TriggersFired(IList<IOperableTrigger> triggers)
        {
            lock (lockObject)
            {
                ...
                foreach (IOperableTrigger trigger in triggers)
                {
                    ...
                    if (job.GroupConcurrentExecutionDisallowed)
                    {
                        List<TriggerWrapper> trigs = GetTriggerWrappersForJobGroup(job.Key.Group);
                                                 ...
                                                 SchedulerInstruction triggerInstCode)
        {
            lock (lockObject)
            {
                ...
                if (jobsByKey.TryGetValue(jobDetail.Key, out jw))
                {
                    ...
                    if (jd.GroupConcurrentExecutionDisallowed)
                    {
                        blockedJobs.Remove(jd.Key);
                        List<TriggerWrapper> trigs = this.GetTriggerWrappersForJobGroup(jd.Key.Group);
                        foreach (TriggerWrapper ttw in trigs)
                        {
