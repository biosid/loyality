...
SELECT [SessionId],[MobilePhone],-1,GETDATE() 
...
WHERE br.[SessionId] = @sessionId
  ...
  SET	[ClientId] = ires.[ClientId],
		[Status] =	CASE 
						WHEN ires.[Status] IS NULL THEN 3 -- NOTE: Почему-то нет обработанного не дубля
						WHEN ires.[Status] = 1 THEN 2 -- NOTE: Не дубль успешно зареген
						ELSE ires.[Status] -- NOTE: Статус не дубля подойдет (уже зареген или что-то случилось)
					END
FROM [dbo].[ClientForBankRegistrationResponse] tab
LEFT JOIN [dbo].[ClientForBankRegistrationResponse] ires 
	ON ires.SessionId = tab.SessionId AND ires.[Login] = tab.[Login] AND ires.[Status] &gt;= 0
WHERE tab.[SessionId] = @sessionId
  AND tab.[Status] &lt; 0</Text>
       <Parameters>
          <EtlQueryParameter>
            <Name>sessionId</Name>
            <Value>$(SessionId)</Value>
...
VALUES (@PackageId, N'RegisterBankClients', @xml, 0, 1)
ELSE
