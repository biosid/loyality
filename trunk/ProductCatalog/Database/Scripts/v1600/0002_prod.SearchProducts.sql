/****** Object:  StoredProcedure [prod].[SearchProducts]    Script Date: 05/13/2014 14:52:25 ******/
SET ANSI_NULLS ON
GO
...
set @whereSql = @whereSql +
		...
		(
			...
			or 
			(not p.[BasePriceRur] is null and p.[BasePriceRur] > p.[PriceRur])
		) 
		' 
...
begin
	...
	where Balance = (select MIN(Balance) from prod.RecommendedPrices where Balance >= @balance)
...
set @searchSql = '
	...
	select 
		...
		,case when t.PriceBase <> t.PriceAction then cast(1 as bit)
			when (not t.[BasePriceRur] is null and t.[BasePriceRUR] > t.[PriceRur]) then cast(1 as bit)
			else cast(0 as bit) 
