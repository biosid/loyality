IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[prod].[CreateCache]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [prod].[CreateCache]
GO

CREATE PROCEDURE [prod].[CreateCache]
...
set @totalPriceSql = @actionSql + ' + ISNULL(' + @deliveryActionSql + ',0) '
