...
--EXEC [prod].[GetCategories] @includeParent=0, @totalCount = 1000, @childrenCount = 1000,@countToTake=1000, @parentId = 486, @nestingLevel = 4
...
IF EXISTS (
	...
	WHERE id=OBJECT_ID('tempdb..#deniedCategories')) 
...
(
	...
	where 
		...
		and pc.Id not in (select CategoryId from #deniedCategories)
)
