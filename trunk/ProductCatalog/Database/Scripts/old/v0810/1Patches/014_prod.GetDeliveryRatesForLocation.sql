...
SELECT [PartnerId],[MinWeightGram],[MaxWeightGram],[PriceRur],MIN([Type]) AS [Type], [Priority], CarrierId, ExternalLocationId
FROM
(
...
GROUP BY [PartnerId],[MinWeightGram],[MaxWeightGram],[PriceRur], [Priority], CarrierId, ExternalLocationId
