...
SELECT [PartnerId],[MinWeightGram],[MaxWeightGram],[PriceRur],MIN([Type]) AS [Type], [Priority], CarrierId, PartnerDeliveryRateId
FROM
(
-- Получаем все тарифы если @kladr это населённый пункт
SELECT [PartnerId],[KLADR],[MinWeightGram],[MaxWeightGram],[PriceRur],1 as [Type], [Priority], CarrierId, PartnerDeliveryRateId
...
SELECT [PartnerId],[KLADR],[MinWeightGram],[MaxWeightGram],[PriceRur],2 as [Type], [Priority], CarrierId, PartnerDeliveryRateId
FROM [prod].[PartnerDeliveryRates]
...
SELECT [PartnerId],[KLADR],[MinWeightGram],[MaxWeightGram],[PriceRur],3 as [Type], [Priority], CarrierId, PartnerDeliveryRateId
FROM [prod].[PartnerDeliveryRates]
...
SELECT [PartnerId],[KLADR],[MinWeightGram],[MaxWeightGram],[PriceRur],4 as [Type], [Priority], CarrierId, PartnerDeliveryRateId
FROM [prod].[PartnerDeliveryRates]
WHERE SUBSTRING(kladr, 1, 2) = SUBSTRING(@kladr, 1, 2) AND CAST(SUBSTRING(kladr, 3, LEN(kladr)-2) AS BIGINT) = 0
) p
GROUP BY [PartnerId],[MinWeightGram],[MaxWeightGram],[PriceRur], [Priority], CarrierId, PartnerDeliveryRateId
