/****** Object:  StoredProcedure [prod].[SearchProducts]    Script Date: 11.07.2013 12:15:30 ******/
SET ANSI_NULLS ON
GO
...
--exec [prod].[SearchProducts] @totalCount=100, @calculatedMaxPrice=1, @locationCode='7700000000000', @onlyPopular = 1, @partnerIds = 2 @productIds = '1_3296691,1_10224224', @targetAudiencesIds = '1,3'
-- @productParamsXml = '
--<ArrayOfProductParam>
--  <ProductParam>
--    <Name>Вес</Name>
...
BEGIN
	...
	set @priceApply = ' APPLY ( select top 1 Price, CarrierId
						...
						where dp.PartnerId = p.PartnerId AND (p.Weight * @productsQuantity) BETWEEN dp.WeightMin AND dp.WeightMax
...
set @whereSql = '
	where
		...
		and
		(@searchTerm is null or LOWER(p.Name + p.ProductId) like ''%'' + LOWER(@searchTerm) + ''%'')
		and
...
set @searchSql = '
	...
	select 
		...
		,t.PriceAction
		,PriceTotal
		,PriceDelivery AS PriceDeliveryItem
		,PriceDelivery * @productsQuantity AS PriceDelivery
		,PriceDeliveryRur
