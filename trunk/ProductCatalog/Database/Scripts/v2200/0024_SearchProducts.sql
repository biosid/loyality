...
-- тип популярности по топ-300 случайных
if (@sort = 9 and @popularType is null)
   set @popularType = 5
...
set @sortCriteria = (case 
	...
	when @sort = 9 then 'case when PopularRate is not null then 1 else 0 end desc, case when @minRecommendedPrice <= price.PriceAction and price.PriceAction <= @maxRecommendedPrice then 1 else 0 end desc, newid()'
