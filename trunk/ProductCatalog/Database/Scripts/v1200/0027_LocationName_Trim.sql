/****** Object:  StoredProcedure [prod].[ImportDeliveryRatesCheckBuffer]    Script Date: 19.03.2014 9:00:00 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	UPDATE [prod].[BUFFER_DeliveryRates]
	   SET 
		[KLADR] = SUBSTRING([KLADR], 1, 13)
	 WHERE [KLADR] IS NOT NULL AND LEN([KLADR]) > 13

	--Делаем trim для LocationName
	UPDATE [prod].[BUFFER_DeliveryRates]
	   SET LocationName = ltrim(rtrim(LocationName))
