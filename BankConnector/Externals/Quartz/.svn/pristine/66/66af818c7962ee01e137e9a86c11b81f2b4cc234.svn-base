...
namespace Quartz.Impl.AdoJobStore
{
    ...
    public class StdAdoConstants : AdoConstants
    {
        ...
        public static readonly string SqlUpdateJobGroupTriggerStatesFromOtherState =
            string.Format(CultureInfo.InvariantCulture,
                "UPDATE {0}{1} SET {2} = @state WHERE {3} = {4} AND {5} = @jobGroup AND {6} = @oldState",
                TablePrefixSubst,
                TableTriggers, ColumnTriggerState, ColumnSchedulerName, SchedulerNameSubst, ColumnJobGroup, ColumnTriggerState);
