  ...
  <Steps>
	...
	<ExecuteQuery>
      ...
      <Source>
		...
		SET @colNamesForSelect = (SELECT distinct '',MAX('' + QUOTENAME([Name]) + '') AS '' + [Name]
		FROM [dbo].ProfileCustomFields 
		FOR XML PATH(''''), TYPE).value(''.'', ''NVARCHAR(MAX)'')
    ...
    <ExecuteQuery>
      ...
      <Source>
        ...
        <ConnectionString>$(DB)</ConnectionString>
        <ProviderName>System.Data.SqlClient</ProviderName>
        <Text>UPDATE [dbo].[LoyaltyClientUpdates]   SET [FirstName] = N''N/A'',[LastName] = N''N/A'',[MiddleName] = N''N/A'',[MobilePhone] = N''N/A'',[Email] = N''N/A''  WHERE [SendEtlSessionId] = @sessionId</Text>
        <Parameters>
          <EtlQueryParameter>
            <Name>sessionId</Name>
            <Value>$(SessionId)</Value>
