  ...
  <xsl:template match="/EtlDump">
    <html xmlns="http://www.w3.org/1999/xhtml">
      ...
      <body>
        ...
        <xsl:if test="HasMoreSessions = 'true'">
          <br />
          <span>
            Внимание: показаны не все сессии, так как был установлен лимит на количество отображаемых в отчете сессий равный <xsl:value-of select="MaxSessionCount" />.
          </span>
        </xsl:if>
        <br />
        <b style="color:#008000;">
          Всего сессий: <xsl:value-of select="count(Sessions/Session)"/>
        </b>
        <xsl:if test="count(Sessions/Session[Status='Failed']) > 0">
          <b style="color:#ff0000;">
            (ошибок: <xsl:value-of select="count(Sessions/Session[Status='Failed'])"/>)
          </b>
        </xsl:if>
    ...
    <table class="EtlSessionsTable" cellpadding="0" cellspacing="0" width="100%">
      ...
      <xsl:for-each select="key('packageId', EtlPackageId)">
        <tr>
          ...
          <td>
            <xsl:choose>
              <xsl:when test="count(descendant::StatisticsMessage) > 0 or count(descendant::ErrorMessage) > 0">
                <xsl:for-each select="Steps/Step[count(StatisticsMessages/StatisticsMessage) > 0 or count(ErrorMessages/ErrorMessage) > 0]">
                  <xsl:if test="Name != ''">
                    <strong>
                  ...
                  <xsl:for-each select="StatisticsMessages/StatisticsMessage">
