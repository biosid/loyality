namespace S22.Imap
{
    ...
    public class ImapClient : IImapClient
    {
        ...
        private Timer noopTimer = new Timer();

        private int pauseRefCount;
        ...
        /// </summary>
        /// <param name="stream">
        ///     A stream to initialize the ImapClient instance
        ///     with.
        /// </param>
        ...
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ...
        public ImapClient(string hostname, int port = 143, bool ssl = false, RemoteCertificateValidationCallback validate = null)
        {
            this.Connect(hostname, port, ssl, validate);
        }
        ...
        private bool hasEvents
        {
            get
            {
                return this.newMessageEvent != null || this.messageDeleteEvent != null;
            }
        }

        #region IImapClient Members
        ...
        /// </summary>
        /// <remarks>
        ///     To probe a server for IMAP IDLE support, the <see cref="Supports" />
        ...
        ///     Thrown if the specified authentication
        ...
        {
            string tag = this.GetTag(), response;
            switch (method)
            {
                ...
                case AuthMethod.Auto:
                    response = this.AuthAuto(tag, username, password);
                    break;
                case AuthMethod.NtlmOverSspi:
                    response = this.SspiAuthenticate(tag, username, password, true);
                    break;
                case AuthMethod.Gssapi:
                    response = this.SspiAuthenticate(tag, username, password, false);
                    break;
                default:
                    response = this.Authenticate(tag, username, password, method.ToString());
                    break;
            }
            ...
            {
                this.capabilities = response.Substring(13).Trim().Split(' ').Select(s => s.ToUpperInvariant()).ToArray();
                response = this.GetResponse();
            }

            if (!this.IsResponseOK(response, tag))
            {
        ...
        public void Logout()
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string bye = this.SendCommandGetResponse(tag + "LOGOUT");

                if (!bye.StartsWith("* BYE"))
                {
                    throw new BadServerResponseException(string.Format(@"Wrong bye message ""{0}""", bye));
                }

                string response = this.GetResponse();
                if (!this.IsResponseOK(response, tag))
                {
                    throw new BadServerResponseException(string.Format(@"Wrong response ""{0}""", response));
                }
        ...
        /// <remarks>
        ///     This is one of the few methods which can be called in a non-authenticated
        ///     state.
        /// </remarks>
        public string[] Capabilities()
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string response = this.SendCommandGetResponse(command);
        ...
        {
            return (this.capabilities ?? this.Capabilities()).Contains(capability, StringComparer.InvariantCultureIgnoreCase);
        }
        ...
        public void RenameMailbox(string mailbox, string newName)
        {
            ...
            lock (this.sequenceLock)
            {
                this.PauseIdling();
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "RENAME " + Util.UTF7Encode(mailbox).QuoteString() + " " + Util.UTF7Encode(newName).QuoteString());
                this.ResumeIdling();
                if (!this.IsResponseOK(response, tag))
                {
                    throw new BadServerResponseException(response);
        ...
        public void DeleteMailbox(string mailbox)
        {
            ...
            lock (this.sequenceLock)
            {
                this.PauseIdling();
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "DELETE " + Util.UTF7Encode(mailbox).QuoteString());
                this.ResumeIdling();
                if (!this.IsResponseOK(response, tag))
                {
                    throw new BadServerResponseException(response);
        ...
        public void CreateMailbox(string mailbox)
        {
            ...
            lock (this.sequenceLock)
            {
                this.PauseIdling();
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "CREATE " + Util.UTF7Encode(mailbox).QuoteString());
                this.ResumeIdling();
                if (!this.IsResponseOK(response, tag))
                {
                    throw new BadServerResponseException(response);
        ...
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ///     Thrown if a list of mailboxes
        ///     could not be retrieved. The message property of the exception contains the
        ///     error message returned by the server.
        /// </exception>
        /// <remarks>
        ///     The mailbox INBOX is a special name reserved to mean "the
        ...
        public string[] ListMailboxes()
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "LIST \"\" \"*\"");
                while (response.StartsWith("*"))
                {
        ...
        public void Expunge(string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "EXPUNGE");
        ...
        ///     as well as quota information) for the specified mailbox.
        /// </summary>
        /// <param name="mailbox">
        ...
        /// </param>
        /// <returns>
        ///     A MailboxInfo object containing status information for the
        ///     mailbox.
        /// </returns>
        /// <exception cref="NotAuthenticatedException">
        ///     Thrown if the method was called
        ...
        /// </exception>
        /// <remarks>
        ///     Not all IMAP servers support the retrieval of quota information. If
        ///     it is not possible to retrieve this information, the UsedStorage and FreeStorage
        ///     properties of the returned MailboxStatus instance are set to 0.
        /// </remarks>
        ...
        public MailboxInfo GetMailboxInfo(string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                if (mailbox == null)
                {
                    mailbox = this.defaultMailbox;
                }

                MailboxStatus status = this.GetMailboxStatus(mailbox);

                // Collect quota information if the server supports it.
                ulong Used = 0, Free = 0;
                if (this.Supports("QUOTA"))
                {
                    MailboxQuota[] Quotas = this.GetQuota(mailbox);
                ...
                MailboxFlag[] flags = this.GetMailboxFlags(mailbox);

                this.ResumeIdling();
                return new MailboxInfo(mailbox, flags, status.Messages, status.Unread, status.NextUID, Used, Free);
            }
        }
        ...
        public uint[] Search(SearchCondition criteria, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                this.PauseIdling();
                this.SelectMailbox(mailbox);
                string tag = this.GetTag(), str = criteria.ToString();
                StringReader reader = new StringReader(str);
                bool useUTF8 = str.Contains("\r\n");
                string line = reader.ReadLine();
                string response = this.SendCommandGetResponse(tag + "UID SEARCH " + (useUTF8 ? "CHARSET UTF-8 " : string.Empty) + line);
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    response = this.SendCommandGetResponse(line);
                }
        ...
        {
            return this.GetMessage(uid, FetchOptions.Normal, seen, mailbox);
        }

        public string GetRawMessage(uint uid, bool seen = true, string mailbox = null)
        {
            return this.GetMessageData(uid, seen, mailbox);
        }
        ...
        ///     messages in a mailbox share the the same UID.
        ///     <para>
        ///         If you need more fine-grained control over which parts of a mail
        ...
        public MailMessage GetMessage(uint uid, FetchOptions options, bool seen = true, string mailbox = null)
        {
            switch (options)
            {
                case FetchOptions.HeadersOnly:
                    return MessageBuilder.FromHeader(this.GetMailHeader(uid, seen, mailbox));
                case FetchOptions.NoAttachments:
                    return this.GetMessage(uid, p => { return p.Disposition.Type != ContentDispositionType.Attachment; }, seen, mailbox);
                case FetchOptions.TextOnly:
                    return this.GetMessage(uid, p => { return p.Type == ContentType.Text; }, seen, mailbox);
                default:
                    return MessageBuilder.FromMIME822(this.GetMessageData(uid, seen, mailbox));
            }
        }
        ...
        /// <param name="uid">The unique identifier of the mail message to retrieve</param>
        /// <param name="callback">
        ///     A delegate which will be invoked for every MIME body
        ...
        public MailMessage GetMessage(uint uid, ExaminePartDelegate callback, bool seen = true, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                this.PauseIdling();
                this.SelectMailbox(mailbox);
                string header = this.GetMailHeader(uid, seen, mailbox);
                MailMessage message = MessageBuilder.FromHeader(header);
                string structure = this.GetBodystructure(uid, mailbox);
                try
                {
                    ...
                    foreach (Bodypart part in parts)
                    {
                        ...
                        if (callback(part))
                        {
                            string content = this.GetBodypart(uid, part.PartNumber, seen, mailbox);
                ...
                {
                    throw new BadServerResponseException("Server returned erroneous " + "body structure:" + structure);
                }
        ...
        {
            return this.GetMessages(uids, FetchOptions.Normal, seen, mailbox);
        }
        ...
        public MailMessage[] GetMessages(uint[] uids, ExaminePartDelegate callback, bool seen = true, string mailbox = null)
        {
        ...
        public MailMessage[] GetMessages(uint[] uids, FetchOptions options, bool seen = true, string mailbox = null)
        {
            List<MailMessage> list = new List<MailMessage>();
            foreach (uint uid in uids)
            {
        ...
        /// <param name="seen">
        ///     Set this to true to set the \Seen flag for the message
        ///     on the server.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox the message will be stored in. If this
        ///     parameter is omitted, the value of the DefaultMailbox property is used to
        ///     determine the mailbox to store the message in.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ///     Thrown if the method was called
        ///     in a non-authenticated state, i.e. before logging into the server with
        ...
        public uint StoreMessage(MailMessage message, bool seen = false, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "APPEND " + Util.UTF7Encode(mailbox).QuoteString() + (seen ? @" (\Seen)" : string.Empty) + " {" + mime822.Length + "}");
                ...
                response = this.SendCommandGetResponse(mime822);
                while (response.StartsWith("*"))
                {
                    response = this.GetResponse();
                }
                ...
                return this.GetHighestUID(mailbox);
            }
        }
        ...
        /// <param name="seen">
        ///     Set this to true to set the \Seen flag for each message
        ///     on the server.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox the messages will be stored in. If this
        ///     parameter is omitted, the value of the DefaultMailbox property is used to
        ///     determine the mailbox to store the messages in.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ///     Thrown if the method was called
        ///     in a non-authenticated state, i.e. before logging into the server with
        ...
        /// </exception>
        /// <returns>
        ///     An array containing the unique identifiers (UID) of the stored
        ///     messages.
        /// </returns>
        /// <remarks>
        ///     A unique identifier (UID) is a 32-bit value assigned to each
        ...
        public uint[] StoreMessages(MailMessage[] messages, bool seen = false, string mailbox = null)
        {
            List<uint> list = new List<uint>();
            foreach (MailMessage m in messages)
            {
                list.Add(this.StoreMessage(m, seen, mailbox));
            }
        ...
        ///     The name of the mailbox to copy the message
        ///     into.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox the message will be copied from. If this
        ///     parameter is omitted, the value of the DefaultMailbox property is used to
        ///     determine the mailbox to operate on.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ...
        ///     exception contains the error message returned by the server.
        /// </exception>
        ...
        public void CopyMessage(uint uid, string destination, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                this.SelectMailbox(mailbox);
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID COPY " + uid + " " + destination.QuoteString());
                while (response.StartsWith("*"))
                {
                    response = this.GetResponse();
                }
        ...
        /// <param name="destination">
        ///     The name of the mailbox to move the message
        ///     into.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox the message will be moved from. If this
        ///     parameter is omitted, the value of the DefaultMailbox property is used to
        ///     determine the mailbox to operate on.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ...
        {
            this.CopyMessage(uid, destination, mailbox);
            this.DeleteMessage(uid, mailbox);
        }
        ...
        /// <param name="mailbox">
        ///     The mailbox the message will be deleted from. If this
        ///     parameter is omitted, the value of the DefaultMailbox property is used to
        ///     determine the mailbox to operate on.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ...
        public void DeleteMessage(uint uid, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                this.SelectMailbox(mailbox);
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID STORE " + uid + @" +FLAGS.SILENT (\Deleted \Seen)");
                while (response.StartsWith("*"))
                {
                    response = this.GetResponse();
                }
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to retrieve the flag
        ///     attributes for.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox the message will be retrieved from. If this
        ...
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ///     Thrown if the mail message flags
        ...
        {
            Dictionary<string, MessageFlag> messageFlagsMapping = new Dictionary<string, MessageFlag>(StringComparer.OrdinalIgnoreCase)
                                                                  {
                                                                      {
            ...
            lock (this.sequenceLock)
            {
                ...
                this.SelectMailbox(mailbox);
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID FETCH " + uid + " (FLAGS)");
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to set the flag
        ///     attributes for.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox that contains the mail message. If this
        ///     parameter is null, the value of the DefaultMailbox property is used to
        ///     determine the mailbox to operate on.
        /// </param>
        /// <param name="flags">
        ///     One or multiple message flags from the MessageFlag
        ///     enumeration.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ///     Thrown if the method was called
        ...
        /// </exception>
        /// <remarks>
        ///     This method replaces the current flag attributes of the message
        ///     with the specified new flags. If you wish to retain the old attributes, use
        ///     the <see cref="AddMessageFlags" /> method instead.
        /// </remarks>
        ...
        public void SetMessageFlags(uint uid, string mailbox, params MessageFlag[] flags)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID STORE " + uid + @" FLAGS.SILENT (" + flagsString.Trim() + ")");
                while (response.StartsWith("*"))
                {
                    response = this.GetResponse();
                }
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to add the flag
        ///     attributes to.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox that contains the mail message. If this
        ...
        /// </exception>
        /// <remarks>
        ///     This method adds the specified set of flags to the existing set of
        ///     flag attributes of the message. If you wish to replace the old attributes, use
        ///     the <see cref="SetMessageFlags" /> method instead.
        /// </remarks>
        /// <seealso cref="GetMessageFlags" />
        /// <seealso cref="SetMessageFlags" />
        /// <seealso cref="RemoveMessageFlags" />
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to remove the flag
        ///     attributes to.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox that contains the mail message. If this
        ...
        /// </exception>
        /// <remarks>
        ///     This method removes the specified set of flags from the existing set of
        ///     flag attributes of the message. If you wish to replace the old attributes, use
        ///     the <see cref="SetMessageFlags" /> method instead.
        /// </remarks>
        /// <seealso cref="GetMessageFlags" />
        ...
        public void Dispose()
        {
            ...
            {
                this.idleThread.Abort();
                this.idleThread = null;
            }

            if (this.idleDispatch != null)
            {
                this.idleDispatch.Abort();
                this.idleDispatch = null;
            }
        ...
        private void Connect(string hostname, int port, bool ssl, RemoteCertificateValidationCallback validate)
        {
            ...
            if (ssl)
            {
                SslStream sslStream = new SslStream(this.stream, false, validate ?? ((sender, cert, chain, err) => true));
                sslStream.AuthenticateAsClient(hostname);
                this.stream = sslStream;
            }

            // Server issues an untagged OK greeting upon connect.
            string greeting = this.GetResponse();
            if (!this.IsResponseOK(greeting))
            {
        ...
        /// </summary>
        /// <param name="tag">
        ///     The tag identifier to use for performing the
        ///     authentication commands.
        /// </param>
        /// <param name="username">
        ///     The username with which to login in to the
        ///     IMAP server.
        /// </param>
        /// <param name="password">
        ///     The password with which to log in to the
        ///     IMAP server.
        /// </param>
        /// <returns>The response sent by the server.</returns>
        /// <remarks>
        ///     The order of preference of authentication types employed
        ///     by this method is Ntlm, Scram-Sha-1, Digest-Md5, followed by
        ///     Cram-Md5 and finally plaintext Login as a last resort.
        /// </remarks>
        [DebuggerHidden]
        private string AuthAuto(string tag, string username, string password)
        {
            string[] methods = new[]
                               {
                                   "Srp", "Ntlm", "ScramSha1", "DigestMd5", "CramMd5"
                               };
            foreach (string m in methods)
            {
                try
                {
                    string response = this.Authenticate(tag, username, password, m);
                    if (this.IsResponseOK(response, tag) || response.StartsWith("* CAPABILITY"))
                    {
                        return response;
                    }
                }
        ...
        {
            return this.SendCommandGetResponse(tag + "LOGIN " + username.QuoteString() + " " + password.QuoteString());
        }
        ...
        /// </param>
        /// <param name="useNtlm">
        ///     True to authenticate using NTLM, otherwise
        ///     GSSAPI (Kerberos) is used.
        /// </param>
        /// <returns>The response sent by the server.</returns>
        private string SspiAuthenticate(string tag, string username, string password, bool useNtlm)
        {
            string response = this.SendCommandGetResponse(tag + "AUTHENTICATE " + (useNtlm ? "NTLM" : "GSSAPI"));

            // If we get a BAD or NO response, the mechanism is not supported.
            if (response.StartsWith(tag + "BAD") || response.StartsWith(tag + "NO"))
            {
                throw new NotSupportedException("The requested authentication " + "mechanism is not supported by the server.");
            }

            using (FilterStream fs = new FilterStream(this.stream, true))
            {
                using (NegotiateStream ns = new NegotiateStream(fs, true))
                {
                    try
                    {
                        ns.AuthenticateAsClient(new NetworkCredential(username, password), null, string.Empty, useNtlm ? ProtectionLevel.None : ProtectionLevel.EncryptAndSign, TokenImpersonationLevel.Delegation);
                    }
                    catch
                    {
                        return string.Empty;
            ...
            while (response.StartsWith("+ "))
            {
                response = this.SendCommandGetResponse(string.Empty);
            }
        ...
        ///     The name of the SASL authentication
        ///     mechanism to use.
        /// </param>
        /// <returns>The response sent by the server.</returns>
        ...
        ///     authentication mechanism is not supported by the server.
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ...
        /// </exception>
        [DebuggerHidden]
        private string Authenticate(string tag, string username, string password, string mechanism)
        {
            SaslMechanism m = SaslFactory.Create(mechanism);
                        
            if (!this.Supports("Auth=" + m.Name))
            {
                throw new NotSupportedException("The requested authentication " + "mechanism is not supported by the server.");
            }
            ...
            string response = this.SendCommandGetResponse(tag + "AUTHENTICATE " + m.Name);
            ...
            while (!m.IsCompleted)
            {
                ...
                response = this.SendCommandGetResponse(response);
            }
        ...
        {
            Interlocked.Increment(ref this.tag);
            return string.Format("xm{0:000} ", this.tag);
        }
        ...
        ///     suffixed by CRLF (as is required by the IMAP protocol) prior to sending.
        /// </param>
        /// <param name="resolveLiterals">
        ///     Set to true to resolve possible literals
        ///     returned by the server (Refer to RFC 3501 Section 4.3 for details).
        /// </param>
        ...
        private string SendCommandGetResponse(string command, bool resolveLiterals = true)
        {
            lock (this.readLock)
            {
                ...
                return this.GetResponse(resolveLiterals);
            }
        }
                ...
                lock (this.readLock)
                {
                    while (true)
                    {
                        ...
                        if (b == Newline)
                        {
                            ...
                            if (resolveLiterals)
                            {
                                s = Regex.Replace(s, @"{(\d+)}$", m => { return "\"" + this.GetData(Convert.ToInt32(m.Groups[1].Value)) + "\"" + this.GetResponse(false); });
                            }
            ...
            using (var mem = new MemoryStream())
            {
                lock (this.readLock)
                {
                    while (byteCount > 0)
                    {
                        ...
                        int read = this.stream.Read(buffer, 0, request);
        ...
        private void SelectMailbox(string mailbox)
        {
            ...
            {
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "SELECT " + Util.UTF7Encode(mailbox).QuoteString());
        ...
        /// </summary>
        /// <param name="mailbox">
        ///     The mailbox to receive the special-use flags for.
        ///     If this parameter is omitted, the value of the DefaultMailbox property
        ///     is used to determine the mailbox to operate on.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ///     Thrown if the method was called
        ///     in a non-authenticated state, i.e. before logging into the server with
        ...
        /// </exception>
        /// <returns>
        ///     An array containing the special-use flags set on the
        ///     mailbox.
        /// </returns>
        /// <remarks>
        ///     This feature is an optional extension to the IMAP protocol and as
        ///     such some servers may not report any flags at all.
        /// </remarks>
        private MailboxFlag[] GetMailboxFlags(string mailbox = null)
        {
            Dictionary<string, MailboxFlag> dictFlags = new Dictionary<string, MailboxFlag>(StringComparer.OrdinalIgnoreCase)
                                                        {
                                                            {
                                                                @"\All", MailboxFlag.AllMail
                                                            }, 
                                                            {
                                                                @"\AllMail", MailboxFlag.AllMail
                                                            }, 
                                                            {
                                                                @"\Archive", MailboxFlag.Archive
                                                            }, 
                                                            {
                                                                @"\Drafts", MailboxFlag.Drafts
                                                            }, 
                                                            {
                                                                @"\Junk", MailboxFlag.Spam
                                                            }, 
                                                            {
                                                                @"\Spam", MailboxFlag.Spam
                                                            }, 
                                                            {
                                                                @"\Trash", MailboxFlag.Trash
                                                            }, 
                                                            {
                                                                @"\Rubbish", MailboxFlag.Trash
                                                            }, 
                                                            {
                                                                @"\Sent", MailboxFlag.Sent
                                                            }, 
                                                            {
                                                                @"\SentItems", MailboxFlag.Sent
                                                            }
                                                        };
            ...
            lock (this.sequenceLock)
            {
                ...
                // (Refer to RFC6154).
                string command = this.Supports("XLIST") ? "XLIST" : "LIST";
                string response = this.SendCommandGetResponse(tag + command + " \"\" " + Util.UTF7Encode(mailbox).QuoteString());
                while (response.StartsWith("*"))
                {
        ...
        private MailboxStatus GetMailboxStatus(string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "STATUS " + Util.UTF7Encode(mailbox).QuoteString() + " (MESSAGES UNSEEN UIDNEXT)");
                while (response.StartsWith("*"))
                {
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to retrieve the mail
        ///     headers for.
        /// </param>
        /// <param name="seen">
        ///     Set this to true to set the \Seen flag for the fetched
        ...
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ///     Thrown if the mail header could
        ///     not be fetched. The message property of the exception contains the error message
        ///     returned by the server.
        /// </exception>
        /// <returns>
        ///     A string containing the raw mail header of the mail message with the
        ///     specified UID.
        /// </returns>
        private string GetMailHeader(uint uid, bool seen = true, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID FETCH " + uid + " (BODY" + (seen ? null : ".PEEK") + "[HEADER])", false);
                while (response.StartsWith("*"))
                {
                    ...
                    if (m.Success)
                    {
                        ...
                        builder.Append(this.GetData(size));
                        response = this.GetResponse();
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to retrieve the body structure
        ///     for.
        /// </param>
        /// <param name="mailbox">
        ///     The mailbox the messages will be retrieved from. If this
        ...
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ///     Thrown if the body structure could
        ///     not be fetched. The message property of the exception contains the error message
        ///     returned by the server.
        /// </exception>
        /// <returns>
        ///     A string containing the raw body structure of the mail message with the
        ///     specified UID.
        /// </returns>
        /// <remarks>
        ///     A body structure is a textual description of the layout of a mail message.
        ///     It is described in some detail in RFC 3501 under 7.4.2 FETCH response.
        /// </remarks>
        private string GetBodystructure(uint uid, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                this.SelectMailbox(mailbox);
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID FETCH " + uid + " (BODYSTRUCTURE)");
                string structure = string.Empty;
                while (response.StartsWith("*"))
                {
                    Match m = Regex.Match(response, @"FETCH \(.*BODYSTRUCTURE \((.*)\).*\)", RegexOptions.IgnoreCase);
                    if (m.Success)
                    {
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to retrieve a MIME body part
        ///     from.
        /// </param>
        /// <param name="partNumber">
        ///     The part number of the body part to fetch as
        ///     is expected by the IMAP server.
        /// </param>
        /// <param name="seen">
        ///     Set this to true to set the \Seen flag for the fetched
        ///     messages on the server.
        ...
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ///     Thrown if the body part could
        ///     not be fetched. The message property of the exception contains the error message
        ///     returned by the server.
        /// </exception>
        /// <returns>
        ///     A string containing the downloaded body part of the mail message
        ///     with the specified UID.
        /// </returns>
        private string GetBodypart(uint uid, string partNumber, bool seen = true, string mailbox = null)
        {
            if (!this.Authed)
            {
                throw new NotAuthenticatedException();
            ...
            lock (this.sequenceLock)
            {
                ...
                StringBuilder builder = new StringBuilder();
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID FETCH " + uid + " (BODY" + (seen ? null : ".PEEK") + "[" + partNumber + "])", false);
                while (response.StartsWith("*"))
                {
                    Match m = Regex.Match(response, @"\* \d+ FETCH .* {(\d+)}");
                    if (m.Success)
        ...
        /// </summary>
        /// <param name="uid">
        ///     The UID of the mail message to retrieve as a MIME/RFC822
        ///     string.
        /// </param>
        /// <param name="seen">
        ///     Set this to true to set the \Seen flag for the fetched
        ...
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ///     Thrown if the mail message data
        ///     could not be fetched. The message property of the exception contains the error
        ///     message returned by the server.
        /// </exception>
        /// <returns>
        ///     A string containing the raw MIME/RFC822 data of the mail message
        ///     with the specified UID.
        /// </returns>
        private string GetMessageData(uint uid, bool seen = true, string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                StringBuilder builder = new StringBuilder();
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "UID FETCH " + uid + " (BODY" + (seen ? null : ".PEEK") + "[])", false);
                while (response.StartsWith("*"))
                {
                    Match m = Regex.Match(response, @"\* \d+ FETCH .* {(\d+)}");
                    if (m.Success)
        ...
        /// </summary>
        /// <param name="mailbox">
        ///     The mailbox to find the highest UID for. If
        ///     this parameter is null, the value of the DefaultMailbox property is
        ///     used to determine the mailbox to operate on.
        /// </param>
        /// <exception cref="NotAuthenticatedException">
        ///     Thrown if the method was called
        ///     in a non-authenticated state, i.e. before logging into the server with
        ...
        /// <remarks>
        ///     The highest UID usually corresponds to the newest message in a
        ///     mailbox.
        /// </remarks>
        private uint GetHighestUID(string mailbox = null)
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                this.SelectMailbox(mailbox);
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "STATUS " + Util.UTF7Encode(this.selectedMailbox).QuoteString() + " (UIDNEXT)");
                uint nextUID = 0;
                while (response.StartsWith("*"))
                {
        ...
        private void StartIdling()
        {
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "IDLE");
            ...
            this.idling = true;
            this.idleThread = new Thread(this.IdleLoop);
            this.idleThread.IsBackground = true;
            this.idleThread.Start();

            // Setup a timer to issue NOOPs every once in a while.
            this.noopTimer.Interval = 1000 * 60 * 10;
            this.noopTimer.Elapsed += this.IssueNoop;
            this.noopTimer.Start();
        }
        ...
        private void IdleLoop()
        {
            if (this.idleDispatch == null)
            {
                this.idleDispatch = new Thread(this.EventDispatcher);
                this.idleDispatch.IsBackground = true;
                this.idleDispatch.Start();
            }
            ...
            while (true)
            {
                try
                {
                    ...
                    this.idleEvents.Enqueue(response);
                }
                catch (IOException e)
                {
        ...
        private void EventDispatcher()
        {
            ...
            while (true)
            {
                string response = this.idleEvents.Dequeue();
                ...
                uint numberOfMessages = Convert.ToUInt32(m.Groups[1].Value), uid = this.GetHighestUID();
                switch (m.Groups[2].Value.ToUpper())
                {
                    case "EXISTS":
                        if (lastUid != uid)
                        {
                            this.newMessageEvent.Raise(this, new IdleMessageEventArgs(numberOfMessages, uid, this));
                        }

                        break;
                    case "EXPUNGE":
                        this.messageDeleteEvent.Raise(this, new IdleMessageEventArgs(numberOfMessages, uid, this));
                        break;
                }
        ...
        private void IssueNoop(object sender, ElapsedEventArgs e)
        {
            lock (this.sequenceLock)
            {
                this.PauseIdling();
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "NOOP");
                while (response.StartsWith("*"))
                {
                    response = this.GetResponse();
                }
        ...
        /// </summary>
        /// <param name="mailbox">
        ///     The mailbox to retrieve QUOTA information for.
        ///     If this parameter is null, the value of the DefaultMailbox property is
        ///     used to determine the mailbox to operate on.
        /// </param>
        /// <returns>
        ///     A list of MailboxQuota objects describing usage and limits
        ...
        /// </exception>
        /// <exception cref="InvalidOperationException">
        ///     Thrown if the IMAP4 QUOTA
        ///     extension is not supported by the server.
        /// </exception>
        /// <exception cref="BadServerResponseException">
        ///     Thrown if the quota operation
        ...
        private MailboxQuota[] GetQuota(string mailbox = null)
        {
            ...
            if (!this.Supports("QUOTA"))
            {
                throw new InvalidOperationException("This server does not support the IMAP4 QUOTA extension");
            }
            ...
            lock (this.sequenceLock)
            {
                ...
                string tag = this.GetTag();
                string response = this.SendCommandGetResponse(tag + "GETQUOTAROOT " + Util.UTF7Encode(mailbox).QuoteString());
                while (response.StartsWith("*"))
                {
                    ...
                    if (m.Success)
                    {
                        try
                        {
                            MailboxQuota Quota = new MailboxQuota(m.Groups[2].Value, uint.Parse(m.Groups[3].Value), uint.Parse(m.Groups[4].Value));
                            Quotas.Add(Quota);
                        }
                        catch
                        {
    ...
    /// </summary>
    /// <param name="part">
    ///     A MIME body part of a mail message which consists of multiple
    ///     parts.
    /// </param>
    /// <returns>
    ///     Return true to include the body part in the returned MailMessage object,
    ///     or false to skip it.
    /// </returns>
