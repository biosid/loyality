...
/****** Object:  StoredProcedure [prod].[SearchProducts]    Script Date: 03/23/2015 13:48:38 ******/
...
set @whereSql = @whereSql +				
		' and ( not exists(select CategoryId from #blackListCatIds where CategoryId = p.CategoryId) )'
end;
...
set @sortCriteria = (case 
	...
	when @sort = 9 then 'case when @minRecommendedPrice <= price.PriceAction and price.PriceAction <= @maxRecommendedPrice then 1 else 0 end desc, PopularRate desc, newid()'
