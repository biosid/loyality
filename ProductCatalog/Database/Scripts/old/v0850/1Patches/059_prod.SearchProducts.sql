...
set @fromSql = '
		FROM 
			...
			inner join '+@productsCacheTable+' price on (p.[ProductId] = price.[ProductId] and price.PriceTotal is not null) '
...
IF (@returnNotDeliveried = 0)
BEGIN
	set @whereSql = @whereSql + ' and price.[PriceDelivery] IS NOT NULL '
END
...
set @searchSql = '
	...
	(
		SELECT
			...
			,price.[PriceAction] + ISNULL(price.[PriceDelivery],0) as PriceTotal
