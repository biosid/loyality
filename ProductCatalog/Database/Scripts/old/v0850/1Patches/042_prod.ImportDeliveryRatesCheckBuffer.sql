...
BEGIN
	...
	--Устанавливаем статус -6 для KLADR имеющих разные значение Населеного пункта (LocationName)	
	;WITH Dublicate AS (
	...
	SELECT	@EtlPackageId,
			...
			CASE
				...
				WHEN [Status] = -3
					THEN 'В диапазоне (LINE ' + CAST([Line] AS nvarchar) + ') имеются не заполненые поля: ' + ISNULL([ExternalLocationId], '') + ';' + ISNULL([KLADR], '') + ';' + ISNULL([LocationName], '') + ';' + ISNULL(CAST([MinWeightGram] AS nvarchar), '') + ';' + ISNULL(CAST([MaxWeightGram] AS nvarchar), '') + ';' + ISNULL(CONVERT(nvarchar, cast([PriceRur] as money)), '')
				WHEN [Status] = -4
