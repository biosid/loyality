...
/* BEGIN: NOTE: Для учета таргетинга товаров */
...
BEGIN
	...
	/* BEGIN: NOTE: Для учета таргетинга товаров */
	;WITH UserProductTargetAudiences AS (
		SELECT pta.*, tpta.TargetAudienceId AS UserTargetAudienceId
		FROM [prod].[ProductTargetAudiences] pta
		LEFT JOIN #productTargetAudiences tpta ON pta.TargetAudienceId = tpta.TargetAudienceId
	)
	/* END */
	...
	from #categories c
		join [prod].[Products] p on (p.CategoryId = c.Id) 
			/* BEGIN: NOTE: Для учета таргетинга товаров */
			AND (@ignoreProductTargerting = 0
			     OR 
					(
						NOT EXISTS (	SELECT 1 
										FROM UserProductTargetAudiences upts 
										WHERE p.[ProductId] = upts.[ProductId])
						OR
						EXISTS (	SELECT 1 
									FROM UserProductTargetAudiences upts 
									WHERE p.[ProductId] = upts.[ProductId] 
									  AND upts.UserTargetAudienceId IS NOT NULL)
					)
				)
			/* END */
		join [prod].[ProductCategoriesPermissions] pcp on pcp.CategoryId = p.CategoryId and pcp.PartnerId = p.PartnerId
	where	(@countPublicProducts = 1 AND p.Status = 1 and p.ModerationStatus = 2)
			 OR
...
BEGIN
	...
	from #categories c
		...
		CROSS APPLY (
			 ...
			 ORDER BY dp.[Priority] ASC) delivery
	where 	(@countPublicProducts = 1 AND p.Status = 1 and p.ModerationStatus = 2)
			 OR
			(@countPublicProducts != 1)
	group by c.Id
