...
begin
	...
	SELECT [PartnerId],[MinWeightGram],[MaxWeightGram],[PriceRur],MIN([Type]) AS [Type], MIN([Priority]) AS [Priority], CarrierId, ExternalLocationId
	FROM
	(
	...
	GROUP BY [PartnerId],[MinWeightGram],[MaxWeightGram],[PriceRur], CarrierId, ExternalLocationId
