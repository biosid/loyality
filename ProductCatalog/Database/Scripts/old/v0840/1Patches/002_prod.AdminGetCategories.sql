...
BEGIN
	...
	BEGIN
		SELECT @NamePath = NamePath
		FROM [prod].[ProductCategories] NOLOCK
		WHERE Id = @parentId
	END 

	-- NOTE: Если необходимо брать только дочки заданой, то символ "_" гарантирует что LIKE не вернет саму заданую категорию
	IF (@includeParent != 1)
	BEGIN
	...
	;WITH 
		...
		WHERE (@NamePath IS NULL OR c.NamePath LIKE @NamePath + '%')
	  ...
	  CategoriesWithSubCategoriesCount AS (
		...
		WHERE (@nestingLevel is null or c1.NestingLevel <= @nestingLevel)
		GROUP BY c1.Id
	  )
	  ...
	  LEFT JOIN prod.Products p ON cwc.Id = p.CategoryId
