...
BEGIN
    ...
    ([PartnerId],[KLADR],[MinWeightGram],[MaxWeightGram],[PriceRur],[PartnerDeliveryRateId])
    SELECT @PartnerId,[KLADR],[MinWeightGram],[MaxWeightGram],[PriceRur],[PartnerDeliveryRateId]
	FROM [prod].[BUFFER_DeliveryRates]
	WHERE [EtlSessionId] = @EtlSessionId AND [Status] = 0
