/****** Object:  StoredProcedure [prod].[SearchProducts]    Script Date: 12/04/2013 21:17:11 ******/
SET ANSI_NULLS ON
GO
...
set @fromSql = '
		FROM
			...
			inner join '+@productsCacheTable+' price on p.[ProductId] = price.[ProductId] 
					and (ISNULL(@returnNotDeliveried,0) = 1 OR price.PriceTotal IS NOT NULL) '
		...
		(
			select *,
					...
					(
						...
						case when @sort = 8 then IsRecommended end desc,
