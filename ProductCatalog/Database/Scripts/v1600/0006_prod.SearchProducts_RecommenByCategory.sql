...
ALTER procedure [prod].[SearchProducts]
	...
	@balanceToRecommendFor money = null,
...
if (@sort = 6 and @popularType is null)
   set @popularType = 3

-- если ищем рекомендации по балансу, но баланс не указан, то возвращаем рекомендации для нулевого баланса
if (@sort = 9 and @balanceToRecommendFor is null)
   set @balanceToRecommendFor = 0
...
declare @minPriceByBalance money
declare @maxPriceByBalance money

if (@sort = 9)
begin
	select @minPriceByBalance = MinPrice, @maxPriceByBalance = MaxPrice  from prod.RecommendedPrices 
	where Balance = (select MIN(Balance) from prod.RecommendedPrices where Balance >= @balanceToRecommendFor)
	
	set @minPriceByBalance = ISNULL(@minPriceByBalance, 0)
	set @maxPriceByBalance = ISNULL(@maxPriceByBalance, 0)	
...
set @searchSql = '
	...
	select 
		t.*
		,CategoryName
		,case when t.PriceBase > t.PriceAction then cast(1 as bit) else cast(0 as bit) end as IsActionPrice
	from
		(
			select 
					row_number() over 
					(
						...
						,case when @sort = 9 then case when @minPriceByBalance <= price.[PriceAction] and price.[PriceAction] <= @maxPriceByBalance then 1 else 0 end end desc
...
EXECUTE sp_executesql 
	...
	@popularType int,
	@minPriceByBalance money,	
	@maxPriceByBalance money,	
	...
	@minPriceByBalance = @minPriceByBalance,
	@maxPriceByBalance = @maxPriceByBalance,
