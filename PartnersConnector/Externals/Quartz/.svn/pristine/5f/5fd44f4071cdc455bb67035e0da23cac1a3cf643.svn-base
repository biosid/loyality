    ...
    public abstract class JobStoreSupport : AdoConstants, IJobStore
    {
        ...
        protected virtual void StoreTrigger(ConnectionAndTransactionHolder conn, IOperableTrigger newTrigger, IJobDetail job, bool replaceExisting, string state, bool forceState, bool recovering)
        {
            ...
            try
            {
                ...
                if (job.GroupConcurrentExecutionDisallowed && !recovering)
                {
       ...
       protected virtual ICalendar RetrieveCalendar(ConnectionAndTransactionHolder conn, string calName)
            ...
            do
            {
                ...
                try
                {
                    ...
                    foreach (TriggerKey triggerKey in keys)
                    {
                        ...
                        if (job.GroupConcurrentExecutionDisallowed)
                        {
                            if (acquiredJobKeysForNoConcurrentExec.Any(x => x.Group.Equals(jobKey.Group)))
                            {
                                continue; // next trigger
                            }
                            else
        ...
        protected virtual TriggerFiredBundle TriggerFired(ConnectionAndTransactionHolder conn, IOperableTrigger trigger)
        {
            ...
            if (job.GroupConcurrentExecutionDisallowed)
            {
                ...
                try
                {
                    Delegate.UpdateTriggerStatesForJobGroupFromOtherState(conn, job.Key, StateBlocked, StateWaiting);
                    Delegate.UpdateTriggerStatesForJobGroupFromOtherState(conn, job.Key, StateBlocked, StateAcquired);
                    Delegate.UpdateTriggerStatesForJobGroupFromOtherState(conn, job.Key, StatePausedBlocked, StatePaused);
                }
                catch (Exception e)
                {
                                                    ...
                                                    IJobDetail jobDetail, SchedulerInstruction triggerInstCode)
        {
            try
            {
                ...
                if (jobDetail.GroupConcurrentExecutionDisallowed)
                {
                    Delegate.UpdateTriggerStatesForJobGroupFromOtherState(conn, jobDetail.Key, StateWaiting, StateBlocked);
                    Delegate.UpdateTriggerStatesForJobGroupFromOtherState(conn, jobDetail.Key, StatePaused, StatePausedBlocked);
                    SignalSchedulingChangeOnTxCompletion(null);
                }
                else
                {
