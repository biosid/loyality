namespace RapidSoft.Loaylty.PromoAction.Mechanics.PredicateEval.Evaluation
{
    ...
    /// <summary>
    /// Реализация проверки операнд1 больше операнд2.
    /// </summary>
    internal class GtEquation : EquationEvalBase
    {
        private readonly ILog log = LogManager.GetLogger(typeof(GtEquation));

        /// <summary>
        /// Initializes a new instance of the <see cref="GtEquation"/> class.
        /// </summary>
        /// <param name="values">
        ...
        /// Фабрика конвертеров.
        /// </param>
        public GtEquation(VariableValue[] values, IEvalStrategySelector factory)
            : base(values, factory)
        {
        }

        /// <summary>
        /// Выполняет проверку операнд1 больше операнд2.
        ...
        protected override bool DoEval(object[] objects)
        {
            if (objects.Length != 2)
            {
                throw new InvalidPredicateFormatException("Уравнение проверки \"больше\" должно содержать 2 переменных/литерала");
            }
        ...
        /// <summary>
        /// Выполняет проверку операнд1 больше операнд2 или трансформацию в SQL проверки входных значений.
        /// Кол-во входный значений должно быть равно 2.
        /// </summary>
        /// <param name="values">
        ...
        protected override EvaluateResult DoEvalExt(VariableValue[] values)
        {
            ...
            return EvaluateResult.BuildTansformed(this.Factory.CreateEquationConverter(equationOperator.gt, values));
        }
    }
}